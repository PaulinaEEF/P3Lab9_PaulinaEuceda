...
adminOtroAs::adminOtroAs(string nom)
{
	fileName=nom;
}

bool adminOtroAs::cerrarEscritura(){
	outputFile.close();
	return !outputFile.is_open();
}
bool adminOtroAs::cerrarLectura(){
	inputFile.close();
	return !inputFile.is_open();
}
bool adminOtroAs::guardarSoldadosAsalto(SoldadosAsalto* sol){
...
bool adminOtroAs::abrirEscritura(int modoEscritura){
	modo = modoEscritura;
	if(modo == 1)
...
bool adminOtroAs::abrirLectura(){
	inputFile.open(fileName.c_str(),std::ios::binary);
			return inputFile.is_open();
	
}

vector<SoldadosAsalto*> adminOtroAs::leerSoldadosAsalto(){
